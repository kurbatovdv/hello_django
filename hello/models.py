...
class Shop(models.Model):
    ...
    filial = models.ForeignKey(
        Filial, on_delete=models.CASCADE, verbose_name='Филиал', blank=True)
    provider = models.ForeignKey(
        Provider, on_delete=models.PROTECT, verbose_name='Провайдер', blank=True)
    speed = models.DecimalField(
        'Скорость', max_digits=4, decimal_places=1, null=True, blank=True)
    ...
    license_astor = models.OneToOneField(
        License_Astor, on_delete=models.PROTECT, null=True, verbose_name='Лицензии Астор', blank=True)
...
class Cartridge_journal(models.Model):
    cartridge = models.ForeignKey(
        Cartridge, on_delete=models.CASCADE, verbose_name='Картридж', blank=False)
    department = models.ForeignKey(
        Department, on_delete=models.CASCADE, verbose_name='Отдел', blank=False)
